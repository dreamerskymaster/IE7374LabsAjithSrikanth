...
def run_evaluate(tracker=None, context_id=None):
    ...
    if tracker and context_id is not None:
        tracker.record_execution(
            exec_type=tracker.evaluation_exec_type,
            properties={"champion": (champion["model_name"], "string")},
            input_ids=[], output_ids=[], context_id=context_id
        )

    return str(summary_path)

@log_stage("Model Registration")
def run_register(tracker=None, context_id=None):
